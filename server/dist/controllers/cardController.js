...
import { generateCardNumber, generateCVV, generateExpiryDate, } from "../utils/cardHelpers.js";
...
export const toggleCardStatus = async (req, res, next) => {
    try {
        ...
        const card = await Card.findOneAndUpdate({ _id: id, userId: req.user.userId }, { isActive }, { new: true }).select("-cardNumber -cvv");
        if (!card) {
